...
//	@Test
	...
	/**
	 * 用Mockito mock userService层
	 * 
	 * 
	 * @throws Exception
	 */
	@Test
	public void loginCheckByMock2() throws Exception {
		request.setRequestURI("/loginCheck.html");
		request.addParameter("userName", "admin"); // ⑥ 设置请求URL及参数
		request.addParameter("password", "123");
		...
		doReturn(true).when(userService).hasMatchUser(request.getParameter("userName"),
				request.getParameter("password"));
		
		doReturn(new User("admin", "123")).when(userService).findUserByUserName("admin");
